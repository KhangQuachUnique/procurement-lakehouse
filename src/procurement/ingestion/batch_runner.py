...
def run_batch_range(
    ...
    *,
    ...
    """Create one batch run_id and execute one daily run per closed source date."""
    run_id = uuid.uuid4().hex
    totals = {"days": 0, "skipped_days": 0, "pages": 0, "items": 0, "errors": 0}
    logger.info("batch_started run_id=%s resource=%s start=%s end=%s", run_id, resource, start, end)
    try:
        ...
        logger.exception("batch_failed run_id=%s resource=%s totals=%s", run_id, resource, totals)
        raise
    logger.info("batch_completed run_id=%s resource=%s totals=%s", run_id, resource, totals)
    return run_id
