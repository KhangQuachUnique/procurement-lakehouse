...
def crawl_contractor_result(
    ...
    *,
    ...
    with MuasamcongClient(token=settings.MUASAMCONG_TOKEN) as client:
        spec = create_contractor_result_spec(client)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Crawl closed-day Muasamcong contractor result data"
    )
...
def main() -> None:
    configure_logging()
    args = parse_args()
    crawl_contractor_result(args.start_date, args.end_date, page_size=args.page_size)
